...
def main(config_dict):
    ...
    if config_dict["magma_prefix"] is not None:
        ...
        # PATCH 2026-05-25: filter MAGMA genes to those present in gene_annot_df.
        # Newer pandas (>=1.0) raises on .loc[] with missing labels; older pops.py
        # assumed older pandas behaviour that NaN-filled. Pre-intersecting keeps
        # all downstream .loc[Y_ids] calls valid without modifying their bodies.
        keep_mask = np.isin(Y_ids, gene_annot_df.index.values)
        n_dropped = (~keep_mask).sum()
        if n_dropped > 0:
            logging.info("Dropping {} MAGMA genes not in gene_annot_df (newer-pandas-loc patch)".format(n_dropped))
            Y = Y[keep_mask]
            Y_ids = Y_ids[keep_mask]
            if covariates is not None:
                covariates = covariates[keep_mask]
            if error_cov is not None:
                error_cov = error_cov[np.ix_(keep_mask, keep_mask)]
